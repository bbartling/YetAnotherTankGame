...
public class EnemyTankSpawner : MonoBehaviour
{
    ...
    private void ApplySpawnProfile(EnemyTankAI ai, int index, int totalCount)
    {
        ...
        ai.preferredDistance = Mathf.Clamp(ai.preferredDistance * Mathf.Lerp(1f, 1.12f, normalizedCount), 180f, 280f);
        ai.retreatDistance = Mathf.Clamp(ai.retreatDistance * Mathf.Lerp(1f, 1.08f, normalizedCount), 90f, ai.preferredDistance - 60f);
        ai.moveForce = Mathf.Min(ai.moveForce, 24f);
        ...
        ai.searchTurnTorque = Mathf.Min(ai.searchTurnTorque, 28f);
        ai.ambushRadius = Mathf.Lerp(16f, 26f, normalizedCount);
        ai.patrolMoveForce = Mathf.Min(ai.patrolMoveForce, 18f);
        ai.patrolTurnTorque = Mathf.Min(ai.patrolTurnTorque, 24f);
